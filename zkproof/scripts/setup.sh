...
# Downloads official Hermez Powers of Tau and generates proving/verification keys
#
# IMPORTANT: This script uses the official Hermez trusted setup ceremony files
# from https://github.com/iden3/snarkjs#7-prepare-phase-2
# These are production-ready ptau files from a real multi-party ceremony.
...
echo ""
echo "================================================"
echo "   Phase 1: Powers of Tau (Hermez Ceremony)"
echo "================================================"
echo ""
log "Phase 1 started"
log "${GREEN}Using official Hermez trusted setup ceremony files${NC}"
log "Source: https://github.com/iden3/snarkjs#7-prepare-phase-2"
echo ""
...
if [ "$CONSTRAINTS" -gt 524288 ]; then
    POWER=20  # 2^20 = 1048576
fi
if [ "$CONSTRAINTS" -gt 1048576 ]; then
...
# ================================================
# DOWNLOAD OFFICIAL HERMEZ PTAU FILE
# ================================================
# Hermez ptau files are from a real trusted setup ceremony
# Available powers: 12-28
# URL format: https://storage.googleapis.com/zkevm/ptau/powersOfTau28_hez_final_{N}.ptau

HERMEZ_PTAU_URL="https://storage.googleapis.com/zkevm/ptau/powersOfTau28_hez_final_${POWER}.ptau"
PTAU_FILE="$PTAU_DIR/hermez_pot${POWER}.ptau"

log "PTAU file path: $PTAU_FILE"
log "Download URL: $HERMEZ_PTAU_URL"

# Expected file sizes (approximate, for validation)
# Using a function for Bash 3.x compatibility (macOS default bash)
get_expected_size() {
    local power=$1
    case $power in
        12) echo 4800000 ;;       # ~4.6 MB
        13) echo 9500000 ;;       # ~9.1 MB
        14) echo 19000000 ;;      # ~18 MB
        15) echo 38000000 ;;      # ~36 MB
        16) echo 76000000 ;;      # ~72 MB
        17) echo 152000000 ;;     # ~145 MB
        18) echo 302000000 ;;     # ~288 MB
        19) echo 604000000 ;;     # ~576 MB
        20) echo 1208000000 ;;    # ~1.1 GB
        21) echo 2416000000 ;;    # ~2.3 GB
        *)  echo 1000000 ;;       # default ~1 MB
    esac
}

MIN_SIZE=$(get_expected_size $POWER)

# Check if ptau file already exists and is valid
PTAU_VALID=false
if [ -f "$PTAU_FILE" ]; then
    PTAU_SIZE=$(stat -f%z "$PTAU_FILE" 2>/dev/null || stat -c%s "$PTAU_FILE" 2>/dev/null || echo "0")
    log "${YELLOW}Found existing PTAU file (${PTAU_SIZE} bytes)${NC}"

    # Basic size validation
    if [ "$PTAU_SIZE" -gt "$MIN_SIZE" ]; then
        log "${GREEN}✓ PTAU file size looks valid, skipping download${NC}"
        log "  $PTAU_FILE"
        PTAU_VALID=true
    else
        log "${RED}✗ PTAU file too small (expected > ${MIN_SIZE} bytes), re-downloading...${NC}"
        rm -f "$PTAU_FILE"
    fi
else
    log "PTAU file does not exist, will download"
fi

# Download if needed
if [ "$PTAU_VALID" = false ]; then
    log "${YELLOW}Downloading official Hermez Powers of Tau (power $POWER)...${NC}"
    log "This is a one-time download from the Hermez trusted setup ceremony."
    log "File will be cached for future use."
    echo ""

    # Download with progress
    if command -v curl &> /dev/null; then
        log_cmd "curl -L -o $PTAU_FILE $HERMEZ_PTAU_URL"
        curl -L --progress-bar -o "$PTAU_FILE" "$HERMEZ_PTAU_URL"
    elif command -v wget &> /dev/null; then
        log_cmd "wget -O $PTAU_FILE $HERMEZ_PTAU_URL"
        wget --show-progress -O "$PTAU_FILE" "$HERMEZ_PTAU_URL"
    else
        log "${RED}Error: Neither curl nor wget found. Please install one of them.${NC}"
        exit 1
    fi
    ...
    if [ -f "$PTAU_FILE" ]; then
        PTAU_SIZE=$(stat -f%z "$PTAU_FILE" 2>/dev/null || stat -c%s "$PTAU_FILE" 2>/dev/null || echo "0")
        if [ "$PTAU_SIZE" -gt "$MIN_SIZE" ]; then
            log "${GREEN}✓ Downloaded successfully (${PTAU_SIZE} bytes)${NC}"
        else
            log "${RED}✗ Download failed or file is corrupted${NC}"
            rm -f "$PTAU_FILE"
            exit 1
        fi
    else
        log "${RED}✗ Download failed - file not created${NC}"
        exit 1
    fi
fi

log "Phase 1 completed (Hermez PTAU ready)"
...
# ============================================================
# ALWAYS DELETE CIRCUIT-SPECIFIC FILES
# These must be regenerated when the circuit changes.
# Only the Powers of Tau (ptau) file is universal and reusable.
# ============================================================
log "${YELLOW}Cleaning up old circuit-specific files...${NC}"
log "  (Powers of Tau is universal and will be kept)"

if [ -f "$BUILD_DIR/circuit_0000.zkey" ]; then
    log "  Deleting: $BUILD_DIR/circuit_0000.zkey"
    rm -f "$BUILD_DIR/circuit_0000.zkey"
fi

if [ -f "$BUILD_DIR/circuit_final.zkey" ]; then
    log "  Deleting: $BUILD_DIR/circuit_final.zkey"
    rm -f "$BUILD_DIR/circuit_final.zkey"
fi

if [ -f "$BUILD_DIR/verification_key.json" ]; then
    log "  Deleting: $BUILD_DIR/verification_key.json"
    rm -f "$BUILD_DIR/verification_key.json"
fi

log "${GREEN}✓ Old circuit-specific files cleaned${NC}"
echo ""

# Generate zkey (always, since we deleted old files)
log "${YELLOW}Step 2.1: Generating zkey (proving key)...${NC}"

# Estimate time based on constraints (with Hermez ptau, much faster than custom)
# ~1 minute per 100k constraints with official ptau
ESTIMATED_SECONDS=$((CONSTRAINTS / 2000))
if [ $ESTIMATED_SECONDS -lt 30 ]; then
    ESTIMATED_SECONDS=30
fi
ESTIMATED_MINUTES=$((ESTIMATED_SECONDS / 60))
if [ $ESTIMATED_MINUTES -lt 1 ]; then
    log "${YELLOW}Estimated time: ~${ESTIMATED_SECONDS} seconds${NC}"
else
    log "${YELLOW}Estimated time: ~${ESTIMATED_MINUTES} minute(s)${NC}"
fi
...
log "  - Expected time: ~1-2 minutes (with Hermez ptau)"
...
echo ""
log "${GREEN}✓ Using Hermez trusted setup ceremony (production-ready ptau)${NC}"
echo "The Powers of Tau file is from a real multi-party ceremony."
echo "For production, you may want additional zkey contributors."
echo ""
