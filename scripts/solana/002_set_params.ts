...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    const [, solanaDestinationChain] =
        await ccExchangeRouterProxyInstance.chainIdMapping(solanaChainId);

    if (solanaDestinationChain != solanaChainId) {
        const tx = await ccExchangeRouterProxyInstance.setChainIdMapping(
            solanaChainId,
            solanaChainId
        );
        await tx.wait(1);
        console.log(
            "Set ChainIdMapping for Solana in CcExchangeRouterProxy: ",
            tx.hash
        );
    } else {
        console.log("ChainIdMapping for Solana is already set");
    }
    ...
    logger
        .color("blue")
        .bold()
        .log("Set Bridge Token Ticker Mappings in CcExchangeRouterProxy ...");

    // Get exchange token (USDC) from config
    const exchangeToken = config.get("usdc_token");
    const currentChainId = config.get("chain_id");

    // Set teleBTC and exchange token mapping for current chain
    const teleBTCTicker = ethers.utils.hexZeroPad(
        ethers.utils.toUtf8Bytes("teleBTC"),
        8
    );
    const usdcTicker = ethers.utils.hexZeroPad(
        ethers.utils.toUtf8Bytes("USDC"),
        8
    );

    // Check and set teleBTC mapping for current chain
    const currentTeleBTCMapping =
        await ccExchangeRouterProxyInstance.bridgeTokenTickerMapping(
            teleBTCTicker,
            currentChainId
        );

    if (
        currentTeleBTCMapping !== ethers.utils.hexZeroPad(teleBTC.address, 32)
    ) {
        const setTeleBTCMappingTx =
            await ccExchangeRouterProxyInstance.setBridgeTokenTickerMapping(
                teleBTCTicker,
                currentChainId,
                ethers.utils.hexZeroPad(teleBTC.address, 32)
            );
        await setTeleBTCMappingTx.wait(1);
        console.log(
            "Set teleBTC bridge token ticker mapping for current chain: ",
            setTeleBTCMappingTx.hash
        );
    } else {
        console.log(
            "teleBTC bridge token ticker mapping for current chain is already set"
        );
    }

    // Check and set USDC mapping for current chain
    const currentUSDCMapping =
        await ccExchangeRouterProxyInstance.bridgeTokenTickerMapping(
            usdcTicker,
            currentChainId
        );
    ...
    if (
        currentUSDCMapping !==
        ethers.utils.hexZeroPad(exchangeToken as string, 32)
    ) {
        const setUSDCMappingTx =
            await ccExchangeRouterProxyInstance.setBridgeTokenTickerMapping(
                usdcTicker,
                currentChainId,
                ethers.utils.hexZeroPad(exchangeToken as string, 32)
            );
        await setUSDCMappingTx.wait(1);
        console.log(
            "Set USDC bridge token ticker mapping for current chain: ",
            setUSDCMappingTx.hash
        );
    } else {
        console.log(
            "USDC bridge token ticker mapping for current chain is already set"
        );
    }

    // Set bridge token ticker mapping for Solana
    // Solana USDC address (EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v) as bytes32
    const solanaUSDCBytes =
        "0xc6fa7af3bedbad3a3d65f36aabc97431b1bbe4c2d2f6e0e47ca60203452f5d61";

    const solanaUSDCMapping =
        await ccExchangeRouterProxyInstance.bridgeTokenTickerMapping(
            usdcTicker,
            solanaChainId
        );

    if (solanaUSDCMapping !== solanaUSDCBytes) {
        const setSolanaUSDCMappingTx =
            await ccExchangeRouterProxyInstance.setBridgeTokenTickerMapping(
                usdcTicker,
                solanaChainId,
                solanaUSDCBytes
            );
        await setSolanaUSDCMappingTx.wait(1);
        console.log(
            "Set USDC bridge token ticker mapping for Solana: ",
            setSolanaUSDCMappingTx.hash
        );
    } else {
        console.log(
            "USDC bridge token ticker mapping for Solana is already set"
        );
    }
    ...
    const maticUSDOracle = config.get(
        "chain_link_oracles.wrapped_native_token_usd"
    );
